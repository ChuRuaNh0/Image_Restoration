...
void FaceDetectorSCRFD::deserializeEngine(){
    std::ifstream file("/data/disk1/hungpham/img-restoration/Plate_Detect/RLP-post-224-224.nms.engine", std::ios::binary);
    if (file.good())
    {
...
int main(){
    ...
    cv::Mat img = cv::imread("/data/disk1/hungpham/img-restoration/Plate_Detect/C/images/Bien-so-xe-49-la-o-dau-Bien-so-Lam-Dong-theo-tung-khu-vuc.jpg");
    ...
    for (int i = 0; i < lst_face_align.size(); ++i){
        ...
        cv::imwrite("plate_" + std::to_string(i) + ".jpg", face_image);
    }
}