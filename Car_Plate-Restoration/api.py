...
GFPGAN.to(device)
checkpoint = torch.load('/data/disk1/hungpham/img-restoration/License-Plate-Restoration/experiments/train_GFPGANv1_512/models/net_g_360000.pth', map_location=lambda storage, loc: storage)
...
def restoration(input_img, size):
    ...
    print("hi4")
    print(result.shape)
    # output = get_concat_h(input_img, result)
    print("hi4")
    # print(type(output))
    # cv2.imwrite('infer/result1.png', output)
    print("hi5")
    return result
...
async def Vehicle_Super_Resolution_GFPGAN(file: UploadFile = File(...)):
    ...
    try:
        ...
        # bgr_img = base64_to_image(image_content)
        ...
        cv2.imwrite('result.jpg', output_plate)
        retval, buffer = cv2.imencode('.jpg', output_plate)
...
if __name__ == "__main__":
    uvicorn.run("api:app", host="172.16.10.239", port=8080, log_level="info")