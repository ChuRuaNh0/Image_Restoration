...
class FFHQDegradationDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # self.perspective_transform_range = opt['perspective_transform_range']
    ...
    def random_mask(img):
        if random.random() > 0.3:
            if random.random() > 0.5:
                ...
                return FFHQDegradationDataset.random_regular_mask(img)
            else:
                # Random inregular mask (prob: 0.35)
                return FFHQDegradationDataset.random_irregular_mask(img)
        else:
    ...
    def get_component_coordinates(self, index, status):
        ...
        # get coordinates
        locations = []
        for part in ['char_0', 'char_1', 'char_2', 'char_3', 'char_4', 'char_5', 'char_6', 'char_7', 'char_8', 'char_9']:
            box = components_bbox[part][0:4]
            loc = torch.from_numpy(np.array(box)).float()
    ...
    def __getitem__(self, index):
        ...
        if self.crop_components:
            ...
            char_0, char_1, char_2, char_3, char_4, char_5, char_6, char_7, char_8, char_9 = locations

        # ------------------------ generate lq image ------------------------ #
        # blur
        img_lq = degradations.random_mixed_kernels(img = img_gt,
                                                ...
                                                noise_range=None,
                                                pad_kernel = True,
                                                pad_kernel_size = self.blur_kernel_size)
        # downsample
        ...
        # # resize x4
        # img_lq = cv2.resize(img_lq, (int(w // scale * 4), int(h // scale * 4)), interpolation= cv2.INTER_LINEAR)

        # # perspective transform
        # if self.perspective_transform_range is not None:
        #     img_lq = degradations.perstranforms(img_lq, self.perspective_transform_range)
