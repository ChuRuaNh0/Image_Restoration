...
class ConstantInput(nn.Module):
    ...
    def __init__(self, num_channel, size_width, size_height):
        super(ConstantInput, self).__init__()
        self.weight = nn.Parameter(torch.randn(1, num_channel, size_height, size_width))
...
class StyleGAN2OCRGenerator(nn.Module):
    """StyleGAN2 Generator.

    Args:
        input_width (int): The width of outputs.
        input_height (int): The height of outputs.
    ...
    def __init__(self,
                 ...
                 lr_mlp=0.01,
                 narrow=1):
        super(StyleGAN2OCRGenerator, self).__init__()
        # Style MLP layers
        ...
        self.constant_input = ConstantInput(channels['4'], size_height=4, size_width=4*int(input_width/input_height))
        ...
        out_size = min(input_width, input_height)
        ...
        # noise
        ratio = int(input_width / input_height)
        for layer_idx in range(self.num_layers):
            resolution_height = 2**((layer_idx + 5) // 2)
            resolution_width  = resolution_height * ratio
            shape = [1, 1, resolution_height, resolution_width]
