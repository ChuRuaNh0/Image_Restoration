...
def sobel_loss(pred, target, reduction = 'none'):
    ...
    train_img = torch.sqrt(torch.abs(train_Gx)+torch.abs(train_Gy))
    ground_img = torch.sqrt(torch.abs(ground_Gx)+torch.abs(ground_Gy))
    # transform  function, three function, f, sqrt(f),pow(f)
    return F.l1_loss(train_img, ground_img, reduction=reduction)

@weighted_loss
def wmse_loss(pred, target, reduction = 'none'):
    batch_size = pred.size()[0]
    channels = pred.size()[1]
    G_X = [[-1, 0, 1],
          [-2, 0, 2],
    ...
    ground_img = torch.sqrt(torch.abs(ground_Gx)+torch.abs(ground_Gy))
    # transform  function, three function, f, sqrt(f),pow(f)
    return F.mse_loss(train_img, ground_img, reduction=reduction)
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        Args:
            ...
            weight (Tensor, optional): of shape (N, C, H, W). Element-wise
                weights. Default: None.
        """
        return self.loss_weight * wmse_loss(pred, target, weight, reduction=self.reduction)
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        Args:
            ...
            weight (Tensor, optional): of shape (N, C, H, W). Element-wise
                weights. Default: None.
        """
        return self.loss_weight * sobel_loss(pred, target, weight, reduction=self.reduction)
